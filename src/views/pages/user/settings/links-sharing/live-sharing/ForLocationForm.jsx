...
export default function ForLocationForm(props) {
  ...
  const addresses = [
    {label: "Park Plaza Warehouse", id: "1"},
    {label: "New York Warehouse", id: "2"},
  ]
  ...
  const validate = (values) => {
    const errors = {};
    if (!values.name) errors.name = 'There must be a name';
    return errors;
  };
          ...
          return (
            <form onSubmit={handleSubmit} noValidate>
              <GridContainer justify="space-between">
                <GridItem xs={12} className={classes.radioLabel + " " + classes.formRow}>
                  Share real-time status of deliveries arriving at an address book location as part of a route.
                </GridItem>
                ...
                <GridItem className={classes.formRow} xs={12}>
                  <InputLabel className={classes.label}>Address book location</InputLabel>
                  <Field
                    ...
                    style={{margin: 0}}
                  >
                    {addresses.map(e => <MenuItem key={e.id} value={e.id}>{e.label}</MenuItem>)}
                  </Field>
                </GridItem>
